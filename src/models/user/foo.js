...
assign(Base.prototype, Events, {
  ...
  set: function(key, value, options) {
    ...
    for (
      var i = 0, keys = Object.keys(attrs), len = keys.length;
      i < len;
      i++
    ) {
      attr = keys[i];
      ...
      if (!def) {
        ...
        if (this._children[attr] || this._collections[attr]) {
          ...
          throw new TypeError(
            'Invalid value for extraProperties: "' + extraProperties + '"'
          );
        }
      }
  ...
  _validate: function(attrs, options) {
    ...
    this.trigger(
      ...
      this,
      error,
      assign(options || {}, { validationError: error })
    );
    return false;
  },
  ...
  _ensureValidType: function(type) {
    return includes(
      [
        ...
        "state",
        "any"
      ].concat(Object.keys(this._dataTypes)),
      type
    )
      ? type
      : undefined;
  ...
  getAttributes: function(options, raw) {
    ...
    if (options.derived) {
      for (var derivedItem in this._derived)
        res[derivedItem] = this[derivedItem];
    }
    return res;
  },
  ...
  _getCachedEventBubblingHandler: function(propertyName) {
    if (!this._eventBubblingHandlerCache[propertyName]) {
      this._eventBubblingHandlerCache[propertyName] = function(
        name,
        model,
        newValue
      ) {
...
function createPropertyDefinition(object, name, desc, isSession) {
  ...
  if (isString(desc)) {
    ...
    if (def.required && def["default"] === undefined && !def.setOnce)
  ...
  if (!type) {
    ...
    console.warn(
      "Invalid data type of `" +
        ...
        "` for `" +
        name +
        "` property. Use one of the default types or define your own"
    );
  }
  ...
  Object.defineProperty(object, name, {
    ...
    get: function() {
      if (!this._values) {
        throw Error(
          'You may be trying to `extend` a state object with "' +
            name +
            '" which has been defined in `props` on the object being extended'
        );
      }
...
function extend(protoProps) {
  ...
  child.prototype._dataTypes = assign(
    {},
